...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Start { name, command, args, workdir, env } => {
            let workdir = workdir.map(PathBuf::from);
            let env_vars = Commands::parse_env_vars(&env)?;
            manager.start_process(name, command, args, workdir, env_vars).await?;
        }
        Commands::Stop { name } => {
        ...
        Commands::Env { name, vars } => {
            ...
            manager.set_env_vars(&name, env_vars)?;
        }
    }
